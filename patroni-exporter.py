...
class PatroniCollector:
    ...
    def scrape_patroni(self) -> None:
        ...
        :return:
        """
        logger.debug('Scraping Patroni API at %s.', {self.url})
        try:
            ...
            self.scrape = {}
            logger.error('Scraping of Patroni @ %s failed: %s', {self.url}, {e})
        logger.debug("Scraped data: {m}".format(m=self.scrape))
    @staticmethod
    def to_timestamp(timestring: str) -> int:
        logger.debug('Converting %s to unix timestamp', {timestring})
    ...
    def preprocessing(self) -> None:
        ...
        for data_key, scrape_key in direct_mapping.items():
            logger.debug('Pre-processing %s', {data_key})
        ...
        for key, values in scrape_mapping.items():
            for item in values:
                logger.debug('Pre-processing %s', {item})
                ...
                if item in {'pending_restart', 'cluster_unlocked', 'pause'} \
                    ...
                    logger.debug('`%s`` not in scrape, setting to False', {item})
        ...
        logger.debug("Preprocessed data: {m}".format(m=self.data))

        if self.scrape:
            logger.warning('Not all metrics '
                           'has been preprocessed: %s', {self.scrape})

    @staticmethod
    def _process_gauge(data: Dict, label: str) -> List[GaugeMetricFamily]:
        metrics = []
        for k, v in data.items():
            g = GaugeMetricFamily('patroni_' + label + '_' + k,
                                  label + ' gauge ' + k)
    ...
    def _process_info(data: Union[Dict, List[Dict]],
        ...
        i = InfoMetricFamily('patroni_' + label,
                             label + ' info')
    ...
    def process_data(self) -> Dict[str, List[Union[GaugeMetricFamily,
        ...
        for key, value in self.data.items():
            ...
            func = getattr(self, '_process_' + func_type, None)
            if not func:
                raise RuntimeError('Metric for {key} cannot be processed. '
    ...
    def collect(self) -> Union[GaugeMetricFamily, InfoMetricFamily]:
        ...
        for name, metrics in sections.items():
            logger.debug('Processing section %s', {name})
...
class PatroniExporter:
    ...
    def parse_args() -> argparse.Namespace:
        ...
        unknown = set(unknown) - {'', ' '}
        if unknown:
            parser.error('Unknown arguments: {", ".join(unknown)}')
        return known
